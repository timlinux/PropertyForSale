...
if ! "$POSTGRES_BIN_DIR/psql" -h "$PGDATA" -p "$PGPORT" -lqt | cut -d \| -f 1 | grep -qw "$DB_NAME"; then
    echo "📦 Creating database $DB_NAME..."
    "$POSTGRES_BIN_DIR/createdb" -h "$PGDATA" -p "$PGPORT" "$DB_NAME"
    "$POSTGRES_BIN_DIR/psql" -h "$PGDATA" -p "$PGPORT" -d "$DB_NAME" -c "CREATE EXTENSION IF NOT EXISTS postgis;"
    "$POSTGRES_BIN_DIR/psql" -h "$PGDATA" -p "$PGPORT" -d "$DB_NAME" -c "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";"
fi
echo "✅ PostgreSQL running (socket: $PGDATA, port: $PGPORT)"
